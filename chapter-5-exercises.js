...
var ancestryMothers = ancestry.filter(function(p) {
  if (p.hasOwnProperty("mother") === false || p.mother === null) {
    return false;
  }
  else {
    return true;
...
var mothersAge = ancestryMothers.map(function(per) {
  return byName[per.mother];
});


console.log(average(mothersAge));
